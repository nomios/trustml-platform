...
export function ProfessionalJourney({
  ...
  return <div className="w-full bg-slate-900 text-white py-20 px-4 sm:px-6 lg:px-8" data-id={dataId}>
                ...
                <div className={`flex items-start bg-slate-800/70 backdrop-blur-sm rounded-xl p-6 shadow-2xl border transition-all duration-300 hover:bg-slate-800/90 hover:scale-[1.02] ${item.isCurrent ? 'border-indigo-500/50 shadow-indigo-500/20' : 'border-slate-600/70'}`}>
                  ...
                  <div className="ml-6 flex-1 min-w-0">
                    ...
                    <div className="mb-3 flex items-center gap-3 flex-wrap">
                      ...
                      {item.isCurrent && <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-indigo-500/50 text-indigo-200 border border-indigo-400/30 animate-pulse">
                          Current Role
                        </span>}
                    </div>
